...
def get_packages(arch: str, packages: str) -> tuple[list[str], list[str]]:
    ...
    child = px.spawn(f'sh -c "cd {os.getcwd()};exec /bin/sh"', encoding="utf-8", timeout=60)  # noqa: E702,E231
...
def add_repos(repos: str):
    ...
    for repo in repos.splitlines():
        ...
        child = px.spawn(f'sh -c "cd {os.getcwd()};exec /bin/sh"', encoding="utf-8", timeout=10)  # noqa: E702,E231
